...
if __name__ == "__main__":
    ...
    with open(csv_path, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["rank", "building", "floor", "area_m2", "price_ils",
                    "est_market_value", "est_profit_ils", "est_profit_pct",
                    "liquidity", "resale_score"])
        for r in ranked:
            w.writerow([r["rank"], r["building"], r["floor"], r["area_m2"],
                        r["price_ils"], r["est_market_value"], r["est_profit_ils"],
    ...
    print(f"{'#':>2} {'bld':>3} {'flr':>3} {'m²':>6} {'price':>9} {'mkt val':>9} "
          f"{'profit':>9} {'%':>5} {'score':>5}")
    for r in ranked[:10]:
        print(f"{r['rank']:>2} {r['building']:>3} {r['floor']:>3} {r['area_m2']:>6.1f} "
              f"{r['price_ils']/1e6:>8.2f}M {r['est_market_value']/1e6:>8.2f}M "
